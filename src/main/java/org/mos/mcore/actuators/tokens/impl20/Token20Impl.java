...
import org.mos.mcore.actuator.exception.TransactionExecuteException;
import org.mos.mcore.actuator.exception.TransactionParameterInvalidException;
import org.mos.mcore.actuators.tokencontracts20.TokensContract20.ContractRC20;
import org.mos.mcore.actuators.tokencontracts20.TokensContract20.ContractRC20.Function20;
...
import org.mos.mcore.actuators.tokens.impl.TConfig;
import org.mos.mcore.actuators.tokens.impl.TokenContractDataAccess;
...
public class Token20Impl implements IActuator {
	...
	public Token20SubAccount loadRC20AccountsFromTouchAccount(ByteString address, ByteString tokenAddr,
		...
		try {
			if (actinfo.getSubAccounts() == null || !actinfo.getSubAccounts().containsKey(tokenAddr)) {
				...
				log.error("token=" + mcore.getCrypto().bytesToHexStr(tokenAddr.toByteArray()) + " tokenbalance=" + subAccount.getBalance());
				actinfo.registerSubBuilder(tokenAddr, subAccount);
			} else {
				Token20SubAccount fromaccount20 = (Token20SubAccount) actinfo.getSubAccounts().get(tokenAddr);
				log.error("token=" + mcore.getCrypto().bytesToHexStr(tokenAddr.toByteArray()) + " exists, tokenBalance=" + fromaccount20.getBalance());
			}
		} catch (Exception e) {
			log.error("error in loadRC20Accounts accounts", e);
		}
	...
	public ByteString batchTransfer(AccountInfoWrapper sender, BigInteger amountList[], Token20SubAccount[] toList,
		...
		if (fromaccount20.zeroSubCheckAndGet(totalAmount).signum() < 0) {
			...
			throw new TransactionParameterInvalidException(
					"balance of the sender is not enough to transfer token ,need:0x" + totalAmount.toString(16) + " have:" + fromaccount20.getBalance());
		}
	...
	public ByteString print(AccountInfoWrapper sender, TransactionInfoWrapper transactionInfo,
		...
		contractAccountW.addAndGet(totalAmount);
	...
	public ByteString burn(AccountInfoWrapper sender, TransactionInfoWrapper transactionInfo,
		...
		if (contractAccountW.zeroSubCheckAndGet(totalAmount).signum() < 0) {
			// 加回去。。。
	...
	public ByteString createContract(AccountInfoWrapper sender, TransactionInfoWrapper transactionInfo,
		...
		if (Function20.CONSTRUCT_PRINTABLE.equals(crc20.getFunction())) {
			...
			for (ByteString manAddr : crc20.getTosList()) {
		...
		ByteString rc20Hash = ByteString.copyFrom(mcore.getCrypto().sha3(rc20InfoBS.toByteArray()));
		AccountInfo.Builder contractAccount = AccountInfo.newBuilder().setAddress(tokenAddress).setExtData(rc20InfoBS)
				.setStatus(0).setNonce(0).setBalance(rc20Info.getTotalSupply()).setStorageTrieRoot(rc20Hash)
				.setType(AccountType.RC20_CONTRACT);
		// 写入touch_info了
		transactionInfo.getTouchAccounts().put(tokenAddress, new AccountInfoWrapper(contractAccount));
		return tokenAddress;
	}
